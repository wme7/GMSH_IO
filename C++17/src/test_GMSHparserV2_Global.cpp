            ...
            else if (ligne.find("$Elements") == static_cast<std::string::size_type>(0))
            {
                ...
                if (phys_MAX_DIM==2) // We assume this is a 2-dimensional simulation problem.
                {
                    ...
                    for (size_t i=0; i<num_elements; ++i)
                    {
                        ...
                        {   
                            ...
                            else if(std::get<1>(gmsh_physicals[physTag])=="axisymmetric")       {BEtype=6;}
                            ...
                            else if(std::get<1>(gmsh_physicals[physTag])=="axisymmetric_rec")       {BEtype=16;}
                            else if(std::get<1>(gmsh_physicals[physTag])=="recordingObject")        {BEtype=17;}
                            else if(std::get<1>(gmsh_physicals[physTag])=="recObj")                 {BEtype=17;}
                            ...
                            if(DEBUG) edge.print();
                            if(BEtype==17) { // if it is an embedded recording object
                        ...
                        {   
                            ...
                            Triangle element(id-one, Nodes[node1-one], Nodes[node2-one], Nodes[node3-one], int(physTag), partitionTag-one);
                        ...
                        {   
                            ...
                            if(DEBUG) std::cout << node1-one << std::endl;
                                 ...
                                 if(std::get<1>(gmsh_physicals[physTag])=="recordingObject")   {BEtype=17;}
                            else if(std::get<1>(gmsh_physicals[physTag])=="recObj")            {BEtype=17;}
                ...
                else if (phys_MAX_DIM==3) // We assume this is a 3-dimensional simulation problem.
                {
                    ...
                    {                        
                        ...
                        {   
                            // Nodes and boundaries type 
                            size_t node1, node2, node3, node4;

                            fichier >> node1 >> node2 >> node3 >> node4;
                            if(DEBUG) std::cout << node1-one << " " << node2-one << " " << node3-one << " " << node4-one << std::endl;

                            Tetrahedron element(id-one, Nodes[node1-one], Nodes[node2-one], Nodes[node3-one], Nodes[node4-one], int(physTag), partitionTag-one);
...
{
    // This is a CPU test
    int root=0, rank=root; std::cout << "CPU word_rank: " << rank << std::endl;
    ...
    { 
        ...
        std::cout << "The ini file is missing" << std::endl;
    } 
    else
    {
        std::string filename = argv[1];
        std::cout << "The ini file is :" << filename << std::endl;
        
        // Get simulation parameters
        parameters=INIparser(filename);
